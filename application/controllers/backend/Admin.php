...
class Admin extends CI_Controller
{
	...
	public function __construct()
	{
		...
		if (!$this->user->loggedin)redirect(site_url("backend/login"));
	...
	public function menus()
	{
		...
		$this->template->loadContent("admin/menus.php", array(
			"menus" => $menus
			)
		);
	}
	...
	public function add_menu()
	{
		$this->template->loadData("activeLink",
			array("admin" => array("menus" => 1)));
		$this->template->loadContent("admin/add_menu.php", array(
	...
	public function add_menu_pro()
	{
		...
		redirect(site_url("backend/admin/menus"));
	}
	...
	public function edit_menu($id)
	{
		...
		$this->template->loadContent("admin/edit_menu.php", array(
			"menu" => $menu,
			"links" => $links
			)
		);
	...
	public function add_custom_field_pro()
	{
		...
		redirect(site_url("backend/admin/custom_fields"));
	}
	...
	public function premium_users_page()
	{
		...
		foreach($users->result() as $r) {
			...
			$this->datatables->data[] = array(
				...
				'<a href="' . site_url("backend/edit_member/" . $r->ID) .'" class="btn btn-warning btn-xs" title="'. lang("ctn_55").'"><span class="glyphicon glyphicon-cog"></span></a> <a href="' . site_url("backend/delete_member/" . $r->ID . "/" . $this->security->get_csrf_hash()) .'" onclick="return confirm(\'' . lang("ctn_86") . '\')" class="btn btn-danger btn-xs" title="'. lang("ctn_57") .'"><span class="glyphicon glyphicon-trash"></span></a>'
			);
		}
	...
	public function add_user_role_pro()
	{
		...
		redirect(site_url("backend/admin/user_roles"));
	}
	...
	public function add_payment_plan()
	{
		...
		redirect(site_url("backend/admin/payment_plans"));
	}
	...
	public function payment_settings_pro()
	{
		...
		redirect(site_url("backend/admin/payment_settings"));
	...
	public function email_members_pro()
	{
		...
		redirect(site_url("backend/admin/email_members"));
	}
	...
	public function add_group_pro()
	{
		...
		redirect(site_url("backend/admin/user_groups"));
	}
	...
	public function view_group($id, $page=0)
	{
		...
		$config['base_url'] = site_url("backend/view_group/" . $id);
	...
	public function add_user_to_group_pro($id)
	{
		...
		redirect(site_url("backend/admin/view_group/" . $id));
	}
	...
	public function edit_email_template_pro($id)
	{
		...
		redirect(site_url("backend/admin/email_templates"));
	}
	...
	public function add_ipblock()
	{
		...
		redirect(site_url("backend/admin/ipblock"));
	}
	...
	public function members_page()
	{
		...
		foreach($members->result() as $r) {
			...
			$this->datatables->data[] = array(
				...
				date($this->settings->info->date_format, $r->joined),
				$provider,
				'<a href="'.site_url("backend/edit_member/" . $r->ID).'" class="btn btn-warning btn-xs" title="'.lang("ctn_55").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-cog"></span></a> <a href="'.site_url("backend/delete_member/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-xs" onclick="return confirm(\''.lang("ctn_317").'\')" title="'.lang("ctn_57").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-trash"></span></a>'
			);
		}
		echo json_encode($this->datatables->process());
	}
	...
	public function edit_member_pro($id)
	{
		...
		redirect(site_url("backend/members"));
	}
	...
	public function social_settings_pro()
	{
		...
		redirect(site_url("backend/admin/social_settings"));
	}
...
/* 		echo $this->db->last_query();
	...
	public function settings_pro()
	{
		...
		$this->session->set_flashdata("globalmsg", lang("success_13"));
		redirect(site_url("backend/admin/settings"));
	}
