...
class Login extends CI_Controller
{
    	...
    	if($this->settings->info->activate_account)
    	{
    		if(!$r->active) {
    			$this->template->error(lang("error_72") . "<a href='".
    				site_url("backend/register/send_activation_code/" . $r->ID . "/" .
    				 urlencode($r->email)).
	...
	public function google_login()
	{
		...
		$client->setRedirectUri(site_url("backend/login/google_login"));
	...
	public function facebook_login()
	{
		...
		$loginUrl = $helper->getLoginUrl(site_url("backend/login/facebook_login_pro"));
	...
	public function twitter_login()
	{
		...
		$request_token = $twitteroauth->oauth("oauth/request_token",
			array("oauth_callback" => site_url("backend/login/twitter_login_pro")));
	...
	public function twitter_login_pro()
	{
		 ...
		 && !empty($_SESSION['oauth_token_secret']))
		{
		    ...
		    redirect(site_url("backend/login/twitter_login"));
		}
	}
	...
	public function forgotpw_pro()
	{
		...
		$email_template->message = $this->common->replace_keywords(array(
			...
			"[SITE_URL]" => site_url(),
			"[EMAIL_LINK]" =>
				site_url("backend/login/resetpw/" . $token . "/" . $user->ID),
		...
		redirect(site_url("backend/login/forgotpw"));
	}
