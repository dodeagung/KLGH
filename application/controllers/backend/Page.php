...
class Page extends CI_Controller 
{
	...
	public function index() 
	{
		...
		$this->template->loadContent("frontpage/index.php", array(
			)
		);
	}
	...
	public function content_page() 
	{
		...
		foreach($pages->result() as $r) {
			$this->datatables->data[] = array(
				...
				'<a href="'.site_url("backend/frontpage/view/" . $r->ID).'">'.$r->title.'</a>',
				$r->summary,
				'<a href="'.site_url("backend/frontpage/view_cat/" . $r->categoryid).'">'.$r->catname."</a>",
				$this->common->get_user_display(array("username" => $r->username, "avatar" => $r->avatar, "online_timestamp" => $r->online_timestamp)),
				'<a href="'.site_url("backend/frontpage/view/" . $r->ID).'" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="'.lang("ctn_448").'"><span class="glyphicon glyphicon-list"></span></a>'
			);
		}
	...
	public function view($id, $page_int =0) 
	{
		...
		if($page->comments && $this->settings->info->comments) {
			...
			$config['base_url'] = site_url("backend/frontpage/view/" . $id);
		...
		$this->template->loadContent("frontpage/view.php", array(
			"page" => $page,
			"comments" => $comments
			)
		);
	...
	public function categories() 
	{
		...
		$this->template->loadContent("frontpage/cats.php", array(
	...
	public function view_cat($id) 
	{
		...
		$this->template->loadContent("frontpage/view_cat.php", array(
			"category" => $category
			)
		);
	...
	public function add_comment($id) 
	{
		...
		redirect(site_url("backend/frontpage/view/" . $id));
	...
	public function delete_comment($id, $hash) {
		...
		redirect(site_url("backend/frontpage/view/" . $comment->pageid));
	}
