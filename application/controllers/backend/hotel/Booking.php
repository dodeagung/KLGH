...
class Booking extends CI_Controller
{
    ...
    public function booking_plan()
    {
    	$this->template->loadExternalCSS(array(
            'booking/css/cssBooking'
        ));
        
        $this->template->loadExternalJs(array(
            'booking/js/jsBooking'
        ));
        $this->template->loadData("activeLink", array(
            "accomodation" => array(
                "room" => 1
        ...
        $this->template->loadContent("booking/booking_plan.php", array());
    ...
    public function update_plan()
    {
        ...
        foreach ($calendarSelectedArray as $calendarSelected) {
        	...
        	if ($roomStatus == 'renovation') {
	       		 $this->room_m->insertUnavailableRoom($roomId,$calendarSelected,'renovation');
	       	}
