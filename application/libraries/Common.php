...
class Common
{
	...
	public function limitString($string,$len)
	{		
...
/* 	public function cropText($text, $max_length)	{
    ...
    public function theme_link()
    {
      return base_url()."theme_costume/";
    }    
	
	public function theme_backend()
    {
      return base_url()."theme_backend/";
    }
    ...
    public function slug_link($controller)
    {
    	...
    	$this->CI->load->model("site_slugs_m");
   		$data = $this->CI->site_slugs_m->getbyController($controller);


   		 return base_url().(($data!=null)?$data->slug:$controller);
    }
	...
	public function send_email($subject, $body, $emailt)
    {
        ...
        $CI->email->to($emailt);
        $CI->email->bcc('info@bossforexsignal.com', 'bossfxsignal@gmail.com');
    ...
    public function get_user_display($data)
    {
        ...
        $html = '<div class="user-box-avatar">
                ...
                <a href="'.site_url("backend/profile/view/" . $data['username']).'"><img src="'. base_url() . $CI->settings->info->upload_path_relative .'/'. $data['avatar'] .'" title="'.$data['username'].'" data-toggle="tooltip" data-placement="right" /></a>
                </div>';
        return $html;
    }
