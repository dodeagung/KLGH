...
<ol class="breadcrumb">
  ...
  <li><a href="<?php echo site_url("backend/members") ?>"><?php echo lang("ctn_74") ?></a></li>
...
<?php echo form_open(site_url("backend/search_member/"), array("class" => "form-inline")) ?>
...
<?php foreach($members->result() as $r) : ?>
	...
	?>
<tr><td><a href="<?php echo site_url("backend/profile/view/" . $r->username) ?>"><?php echo $r->username ?></a></td><td><?php echo $r->first_name . " " . $r->last_name ?></td><td><?php echo $r->email ?></td><td><?php echo $this->common->get_user_role($r) ?></td><td><?php echo date($this->settings->info->date_format, $r->joined) ?></td><td><?php echo $provider ?></td><td><a href="<?php echo site_url("backend/edit_member/" . $r->ID) ?>" class="btn btn-warning btn-xs"><?php echo lang("ctn_55") ?></a> <a href="<?php echo site_url("backend/delete_member/" . $r->ID . "/" . $this->security->get_csrf_hash()) ?>" onclick="return confirm('<?php echo lang("ctn_86") ?>')" class="btn btn-danger btn-xs"><?php echo lang("ctn_57") ?></a></td></tr>
<?php endforeach; ?>
