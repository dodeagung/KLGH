...
<div class="db-header clearfix">
    ...
    <div class="db-header-extra form-inline">
    	<?php if($this->common->has_permissions(array("admin", "content_manager"), $this->user) || ($this->user->loggedin && $page->userid == $this->user->info->ID) ) : ?>
	    	<a href="<?php echo site_url("backend/content/edit_page/" . $page->ID) ?>" class="btn btn-warning btn-xs" title="<?php echo lang("ctn_55") ?>" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-cog"></span></a> <a href="<?php echo site_url("backend/content/delete_page/" . $page->ID . "/" . $this->security->get_csrf_hash()) ?>" class="btn btn-danger btn-xs" onclick="return confirm('<?php echo lang("ctn_317") ?>')" title="<?php echo lang("ctn_57") ?>" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-trash"></span></a>
	    <?php endif; ?>
    </div>
</div>
...
<div class="media">
  ...
  <div class="media-body">
  	...
  	<a href="<?php echo site_url("backend/page/delete_comment/" . $r->ID . "/" . $this->security->get_csrf_hash()) ?>" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="bottom" title="<?php echo lang("ctn_57") ?>"><span class="glyphicon glyphicon-trash"></span></a>
  	</div>
  <?php endif; ?>
    <?php echo $r->comment ?>
...
<?php echo form_open(site_url("backend/page/add_comment/" . $page->ID), array("class" => "form-horizontal")) ?>
